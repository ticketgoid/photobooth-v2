...
ipcMain.handle('save-settings', (event, data) => {
    const stmt = db.prepare(`
        UPDATE settings 
        SET nama_event=?, saldo_awal=?, hpp_kertas=?, hpp_tinta=?, biaya_ops=?, midtrans_server_key=?, midtrans_client_key=? 
        WHERE id=1
    `);
    stmt.run(data.nama_event, data.saldo_awal, data.hpp_kertas, data.hpp_tinta, data.biaya_ops, data.midtrans_server_key || '', data.midtrans_client_key || '');
    return true;
});
...
ipcMain.handle('create-qris', async (event, amount) => {
    // Inisialisasi Dinamis: Ambil key dari database
